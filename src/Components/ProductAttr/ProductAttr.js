...
<div id={type}>
    ...
    <p><strong>Please, provide {type}'s  {attrs.name} in {attrs.units}</strong></p>
</div>;
}
else {
attrHTML = attrs&&
<div id={type}>
    ...
    <div>
    <label htmlFor={attrs.name}>{attrs.name}({attrs.units})</label>
    <input type="number" min="1" step="0.1" id={attrs.name} required className="spec-attr"/>
    </div>
    <p><strong>Please, provide {type}'s  {attrs.name} in {attrs.units}</strong></p>
</div>;
}
